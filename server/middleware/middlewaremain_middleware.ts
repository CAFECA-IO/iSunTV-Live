  ...
  use(req: IncomingMessage, res: ServerResponse, next: any) {
    ...
    if (req.url.slice(0, 4) === '/api') {
      ...
      if (req.url.slice(0, 13) === '/api/v1/i18n/') {
        if (
          req.url.replace('/api/v1/i18n/', '') != 'ch' &&
          req.url.replace('/api/v1/i18n/', '') != 'en'
        ) {
          res.writeHead(200, {'content-type': 'application/json'});
          res.write(
            JSON.stringify(
              FormatterService.formatData(
                false,
                ERROR_CODE.API_NOT_SUPPORT_ERROR,
                'api not support',
                {}
              )
            )
          ...
          res.writeHead(200, {'content-type': 'application/json'});
          res.write(
            JSON.stringify(FormatterService.formatData(true, 'developing now', '0802xxxx', {}))
          );
          res.end();
        }
